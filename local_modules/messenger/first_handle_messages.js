...
if  (text.includes("hi")) {
    ...
    application.get(`/${sender_psid}/${documents.length - 1}_translation`, function(request, response) {
       if (request.headers.referer) {
        let check = request.headers.referer;
        let check2 = request.route.path;
        if (check.includes("facebook.com") && check2.includes(sender_psid) || check.includes("messenger.com") && check2.includes(sender_psid)){
        response.render(`${sender_psid}/${documents.length - 1}_translation`);
        } else {
